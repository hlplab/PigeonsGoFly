...
# Preamble
Run this to include the necessary dependencies.
...
for (f in list.files("../data/SpeedCircleEnd/", pattern = "*.csv", full.names = TRUE, recursive=TRUE)) {
...
# Load all solo releases

```{r load_all_solo_releases}
GOAL <- c(616070, 5738228)
d <- tibble()

for (f in list.files("../data/Solo_SpeedCircleEnd/", pattern = "*.csv", full.names = TRUE, recursive=TRUE)) {
  bird_id <- sub(".*Solo_SpeedCircleEnd/([^/]+)/.*", "\\1", f)
  release <- as.numeric(sub(".*release_(\\d+).*", "\\1", f))
  if (bird_id %in% c('B77', 'B85', 'B96')) { # Taking a subset of 2 random birds.
      d %<>% bind_rows(read_csv(f) %>% 
                     mutate(bird = bird_id, release = release))
  }
}

# Down sample data to 0.5Hz
d %<>% 
  group_by(bird, release) %>%
  filter(row_number() %% 10 == 0) %>%
  ungroup()

d %<>% 
  rename(x = easting, y=northing, t=time) %>%
  select(x, y, t, bird, release) %>%
  group_by(bird, release) %>%
  mutate(
    next_heading=atan2(lead(y, 1)-y, lead(x, 1)-x),
    heading=lag(next_heading, 1)) %>%
...
d %>%
  ggplot(aes(x, y, color=interaction(bird, release))) + 
  geom_path(aes(group=interaction(bird, release))) + 
  theme(legend.position="none")
d %>%
  ggplot(aes(t, mu_goal, color=interaction(bird, release))) + 
  geom_line(aes(group=interaction(bird, release))) + 
  theme(legend.position="none")
```
  ...
  brm(
    formula = 
      bf(
        ...
        kappa1 ~ 1,
        theta1 ~ distance_to_goal * release + (1 + release|bird),
        # Social direction
